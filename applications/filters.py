...
class MembershipApplicationFilter(django_filters.FilterSet):
    ...
    year_graduated = django_filters.CharFilter(field_name='year_graduated', lookup_expr='exact')
    ...
    def filter_degree_program(self, queryset, name, value):
        ...
        if not value:
            return queryset
        return queryset.filter(degree_program__name__icontains=value)
...
class RejectedApplicationFilter(MembershipApplicationFilter):
    ...
    rejected_from = django_filters.DateFilter(field_name='rejected_at', lookup_expr='gte')
